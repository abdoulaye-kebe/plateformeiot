...
export default function DevicesPage() {
  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      {showForm && write && (
        <form onSubmit={createDevice} className="mb-8 grid gap-3 rounded-xl border border-gray-200 bg-white p-6 sm:grid-cols-2">
          ...
          <button type="submit" className="sm:col-span-2 rounded-lg bg-brand py-2 font-medium">Créer device</button>
        ...
        {devices.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-600 border-b border-gray-200"><th className="pb-2">Nom</th><th>DevEUI</th><th></th></tr></thead>
              <tbody>
                {devices.map((d) => {
                  const eui = devEuiOf(d);
                  const name = d.name ?? (d.device as { name?: string })?.name ?? eui;
                  return (
                    <tr key={eui} className="border-b border-gray-200/50">
                      <td className="py-3 font-medium">{name}</td>
                      <td className="font-mono text-xs text-gray-500">{eui}</td>
                      <td className="text-right"><Link href={`/devices/${eui}`} className="text-brand hover:underline">Détails →</Link></td>
                    </tr>
                  );
                })}
              </tbody>
