    ...
    <div className="p-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Section title="Trafic horaire">
          {traffic.length === 0 ? (
            <p className="text-sm text-slate-500">Aucune donnée — vérifiez que vos devices envoient du trafic et que l&apos;intégration MQTT est active.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              {traffic.slice(-12).map((p) => (
                <li key={p.bucket} className="flex justify-between text-slate-300">
        ...
        <Section title="Règles liées">
          <ul className="space-y-2 text-sm">
            {rules.map((r) => (
              <li key={r.id} className="rounded-lg border border-slate-800 p-3">
                <p className="font-medium">{r.name}</p>
                <p className="text-slate-500">{r.description || "—"}</p>
              </li>
            ))}
          </ul>
          <Link href="/rules" className="mt-3 inline-block text-xs text-emerald-400 hover:underline">Gérer les règles →</Link>
        </Section>
      </div>
    </div>
  );
