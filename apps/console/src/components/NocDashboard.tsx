...
export default function NocDashboard() {
  ...
  if (loading) {
    return <p className="p-8 text-slate-400">Chargement NOC…</p>;
  }

  const widgets = [
    { label: "Uplinks 24h", value: overview?.totalUplinks24h ?? 0, tone: "text-white" },
    { label: "Devices actifs", value: overview?.activeDevices24h ?? 0, tone: "text-emerald-300" },
    { label: "Gateways", value: overview?.activeGateways24h ?? 0, tone: "text-sky-300" },
    {
      label: "RSSI moyen",
      value: overview?.avgRssi24h != null ? `${overview.avgRssi24h.toFixed(1)} dBm` : "—",
      tone: overview?.avgRssi24h != null && overview.avgRssi24h < -115 ? "text-amber-300" : "text-white",
    },
  ];
  ...
  return (
    <div className="p-8">
      ...
      <section className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {widgets.map((w) => (
          <article key={w.label} className="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow-lg shadow-black/20">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          ...
          {alerts.length === 0 ? (
            <p className="text-sm text-slate-500">Aucune alerte — les règles RSSI déclencheront des events ici.</p>
          ) : (
            <ul className="space-y-2">
              {alerts.map((a) => (
                <li key={a.id} className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm">
                  <div>
                    <p className="font-medium text-amber-200">{a.ruleName || "Règle"}</p>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          ...
          {billing ? (
            <dl className="mt-4 space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <dt className="text-slate-400">Uplinks</dt>
              ...
              <div className="flex justify-between border-t border-slate-800 pt-3">
                ...
                <dd className="text-lg font-semibold text-emerald-300">{billing.estimatedEur} €</dd>
              </div>
            </dl>
          ) : (
            <p className="mt-4 text-sm text-slate-500">Données billing indisponibles (rôle tenant-admin requis).</p>
          )}
        </div>
      </section>
    </div>
