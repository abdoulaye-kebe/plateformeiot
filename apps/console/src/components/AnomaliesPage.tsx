  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      <section className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Anomalies ouvertes" value={summary?.openCount ?? 0} />
        <StatCard label="Critiques" value={summary?.bySeverity?.critical ?? 0} />
      ...
      <Section title="Événements détectés">
        {anomalies.length === 0 ? (
          <p className="text-sm text-gray-500">Aucune anomalie — le worker analyse le trafic toutes les 5 minutes.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {anomalies.map((a) => (
                <tr key={a.id} className="border-b border-gray-200/50">
                  <td className={`py-2 capitalize ${SEVERITY_COLOR[a.severity] ?? ""}`}>{a.severity}</td>
                  <td>{a.title}</td>
                  <td className="font-mono text-xs">{a.devEui || "—"}</td>
                  <td className="text-gray-600">{a.anomalyType}</td>
                  <td className="text-gray-600">{new Date(a.detectedAt).toLocaleString("fr-FR")}</td>
                  <td>
                    {!a.resolvedAt && write && (
                      <button type="button" onClick={() => resolve(a.id)} className="text-xs text-brand hover:underline">
                        Résoudre
                      </button>
                    )}
                  </td>
